...
public class LibraryBrowseActivity extends FragmentActivity
implements ViewPager.OnPageChangeListener {
	...
	public ServiceConnection connection = new ServiceConnection() {
		public void onServiceConnected(ComponentName className, final IBinder service) {
			ThreadExecutor.runTask(new Runnable() {
				...
				public void run() {
					...
					if (listeners.size() > 0)
						for (ConnectionListener l : listeners) l.onServiceConnected();
					isConnected = true;
		...
		public void onServiceDisconnected(ComponentName arg0) {
			for (ConnectionListener l : listeners) l.onServiceDisconnected();
			backend = null;
			session = null;
			library = null;
		}
	...
	public void onCreate(Bundle saved) {
		...
		if (!isTablet) {
			...
			getSupportFragmentManager().beginTransaction()
				.add(R.id.frame_artists, artists)
				.add(R.id.frame_albums, albums)
				.add(R.id.frame_playlists, playlists)
				.commit();
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
			...
			if (!isTablet) {
				...
				Tab artTab = ab.newTab().setText(
						R.string.control_menu_artists).setTabListener(handler);
				Tab albTab = ab.newTab().setText(
						R.string.control_menu_albums).setTabListener(handler);
				Tab plyTab = ab.newTab().setText(
						R.string.control_menu_playlists).setTabListener(handler);
				ab.addTab(artTab);
				ab.addTab(albTab);
				ab.addTab(plyTab);
			}
	...
	public class LibraryPagerAdapter extends FragmentStatePagerAdapter {
		...
		public Fragment getItem(int position) {
			if (position == 0) return artists;
			if (position == 1) return albums;
			return playlists;
		}
	...
	public class TabHandler implements ActionBar.TabListener {
		...
		public void onTabSelected(Tab tab, FragmentTransaction ft) {
			if (!fudge) pager.setCurrentItem(tab.getPosition(), true);
			fudge = false;
		}
