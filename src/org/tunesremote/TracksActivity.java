...
public class TracksActivity extends BaseBrowseActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (playlistId != null || allAlbums) setContentView(R.layout.gen_list);
		else {
		...
		public Bitmap doInBackground(Integer... params) {
			...
			try {
				...
				byte[] raw = RequestHelper.request(String.format(
						"%s/databases/%d/groups/%d/extra_data/artwork?session-id=%s&mw=" + coverSize + 
						"&mh=" + coverSize + "&group-type=albums",
						session.getRequestBase(), session.databaseId, params[0], session.sessionId), false);
