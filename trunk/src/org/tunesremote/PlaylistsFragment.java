...
implements ConnectionListener, OnItemClickListener {
	...
	public void onServiceConnected() {
		...
		if (host != null) {
			if (adapter == null) adapter = new PlaylistsAdapter(host, resultsUpdated);
	...
	public void onCreate(Bundle saved) {
		...
		if (adapter == null) adapter = new PlaylistsAdapter(getActivity(), resultsUpdated);
	...
	public Handler resultsUpdated = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (isResumed() && host.positionViewed == 2) setListShown(true);
