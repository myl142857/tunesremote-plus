	...
	public ServiceConnection connection = new ServiceConnection() {
		public void onServiceConnected(ComponentName className, final IBinder service) {
			...
			try {
				...
				if (session == null) {
					if (nHelper != null) nHelper.killNotification();
					return;
				}
...
//								.putString(MediaMetadataRetriever.METADATA_KEY_ARTIST, status.getTrackArtist())
//						.putString(MediaMetadataRetriever.METADATA_KEY_TITLE, status.getTrackName())
//						.putString(MediaMetadataRetriever.METADATA_KEY_ALBUM, status.getTrackAlbum()).apply();
//					}
			}
...
//			ComponentName lockscreenRec = new ComponentName(this, LockscreenReceiver.class);
//			am.registerMediaButtonEventReceiver(lockscreenRec);
//
//			Intent broadcast = new Intent(Intent.ACTION_MEDIA_BUTTON).setComponent(lockscreenRec);
//			PendingIntent broadPender = PendingIntent.getBroadcast(this, 0, broadcast, PendingIntent.FLAG_UPDATE_CURRENT);
//
//			rcc = new RemoteControlClient(broadPender);
//			rcc.setTransportControlFlags(RemoteControlClient.FLAG_KEY_MEDIA_NEXT | RemoteControlClient.FLAG_KEY_MEDIA_PLAY_PAUSE);
	...
	public class NotificationHelper {
		...
		public void updateNotification(String track, String artist, int playState) {
			...
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
				...
				newBuilder.setSmallIcon(R.drawable.ic_notify)
						.setContentTitle(track)
						.setContentText(artist)
						.setDeleteIntent(endServicePending)
						.setContentIntent(launchPending)
						.addAction(playState == Status.STATE_PAUSED ?
								R.drawable.ic_notify_play : R.drawable.ic_notify_pause,
								mContext.getString(playState == Status.STATE_PAUSED ?
										R.string.notify_play : R.string.notify_pause), pausePending)
						.addAction(R.drawable.ic_notify_next, mContext.getString(R.string.notify_next), nextTrackPending);
				if (cover != null) newBuilder.setLargeIcon(cover);
				...
				notification.setImageViewResource(R.id.notify_play, playState == Status.STATE_PAUSED ?
						R.drawable.ic_notify_play : R.drawable.ic_notify_pause);
				...
				builder.setContent(notification)
						.setOngoing(true)
						.setContentIntent(launchPending)
