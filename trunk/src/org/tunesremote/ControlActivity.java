...
public class ControlActivity extends Activity implements
	...
	protected TextView trackName, trackArtist, trackAlbum, seekPosition,
			seekRemain;
	protected SeekBar seekBar;
	protected ImageView coverImage;
	protected ImageButton controlPrev, controlPause, controlNext,
			controlShuffle, controlRepeat;
				...
				public void run() {
					try {
						...
						SharedPreferences settings = PreferenceManager
								.getDefaultSharedPreferences(ControlActivity.this);
						...
						if (session == null) {
							...
							ControlActivity.this.startActivityForResult(
									new Intent(ControlActivity.this,
											LibraryActivity.class), 1);

						} else {
							// for some reason we are not correctly disposing of
							// the
							// session threads we create so we purge any
							// existing ones
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
				...
				case Status.UPDATE_COVER:
					...
					boolean shouldUpdate = (!status.albumId.equals(showingAlbumId))
							&& !status.coverEmpty;
					if (forced)
						shouldUpdate = true;
					...
					Log.d(TAG, String.format(
							"Artwork for albumid=%s, value=%s, what=%d",
							status.albumId, status.coverCache, msg.what));
					if (shouldUpdate) {
						if (status.coverEmpty) {
							// fade down if no coverart
							coverImage.setImageDrawable(new ColorDrawable(
									Color.BLACK));
							...
							if (one != null) {
								...
								coverImage.setImageDrawable(new BitmapDrawable(
										getResources(), status.coverCache));
							}
							one = null;
						}
				...
				case Status.UPDATE_PROGRESS:
					...
					seekPosition
							.setText(Response.convertTime(status.getProgress() * 1000));
					seekRemain.setText("-"
							+ Response.convertTime(status.getRemaining() * 1000));
					if (!dragging) {
			...
			if (ControlActivity.this.autoPause) {
				switch (state) {
					case android.telephony.TelephonyManager.CALL_STATE_IDLE:
						if (wasPlaying
								&& session != null
								&& ControlActivity.status.getPlayStatus() == Status.STATE_PAUSED) {
							session.controlPlay();
							wasPlaying = false;
						}
						break;
					case android.telephony.TelephonyManager.CALL_STATE_OFFHOOK:
						if (session != null
								&& ControlActivity.status.getPlayStatus() == Status.STATE_PLAYING) {
	...
	public void onStart() {
		...
		this.fadeUpNew = this.prefs.getBoolean(
				this.getString(R.string.pref_fadeupnew), this.fadeUpNew);
		this.vibrate = this.prefs.getBoolean(
				this.getString(R.string.pref_vibrate), this.vibrate);
		this.autoPause = this.prefs.getBoolean(
				this.getString(R.string.pref_autopause), this.autoPause);
		this.showRatingBox = this.prefs.getBoolean(
				this.getString(R.string.pref_showrating), true);
		this.showToast = this.prefs.getBoolean(
				this.getString(R.string.pref_showtoast), true);
		this.invertGestures = this.prefs.getBoolean(
				getResources().getString(R.string.pref_invertgestures), false);
	...
	public class VolumeSeekBarListener implements OnSeekBarChangeListener {
			...
			ThreadExecutor.runTask(new Runnable() {
				public void run() {
					try {
						...
						status.setSpeakerVolume(speaker.getId(), newVolume,
								formerVolume, maxVolume, secondMaxVolume,
								cachedVolume);
		...
		public void onProgressChanged(SeekBar seekBar, int newVolume,
		                              boolean fromUser) {
		}
	}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			try {
				...
				TextView nameTextview = (TextView) row
						.findViewById(R.id.speakerNameTextView);
				TextView speakerTypeTextView = (TextView) row
						.findViewById(R.id.speakerTypeTextView);
				final CheckBox activeCheckBox = (CheckBox) row
						.findViewById(R.id.speakerActiveCheckBox);
				SeekBar volumeBar = (SeekBar) row
						.findViewById(R.id.speakerVolumeBar);
				...
				speakerTypeTextView
						.setText(speaker.isLocalSpeaker() ? R.string.speakers_dialog_computer_speaker
				...
				if (speaker.isActive()) {
					...
					volumeBar
							.setOnSeekBarChangeListener(new VolumeSeekBarListener(
									speaker));
				} else {
	...
	protected Dialog onCreateDialog(int id) {
		if (id == DIALOG_SPEAKERS) {
			...
			return new AlertDialog.Builder(this)
					...
					.setTitle(R.string.control_menu_speakers)
					.setAdapter(speakersAdapter, null)
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (!this.agreed) {
			// show eula wizard
			this.startActivityForResult(new Intent(this, WizardActivity.class),
					1);
		}
		...
		this.shuffleToast = Toast.makeText(this,
				R.string.control_menu_shuffle_off, Toast.LENGTH_SHORT);
		this.shuffleToast.setGravity(Gravity.CENTER, 0, 0);

		this.repeatToast = Toast.makeText(this,
				R.string.control_menu_repeat_none, Toast.LENGTH_SHORT);
	...
	public void initViews() {
		...
		this.cropImage = this.prefs.getBoolean(
				this.getString(R.string.pref_cropimage), true);
		if (this.cropImage) {
			this.coverImage.setScaleType(ImageView.ScaleType.CENTER_CROP);
		} else {
			this.coverImage.setScaleType(ImageView.ScaleType.FIT_XY);
		...
		if (!gestureLib.load()) {
			Toast.makeText(this, "Unable to load gestures", Toast.LENGTH_SHORT)
					.show();
		}
		...
		ThreadExecutor.runTask(new Runnable() {
			public void run() {
				...
				runOnUiThread(new Runnable() {
					...
					public void run() {
						try {
							ControlActivity.this.volumeBar
									.setProgress((int) ControlActivity.this.cachedVolume);
							ControlActivity.this.volume.invalidate();
							ControlActivity.this.volumeToast.show();
						} catch (Exception e) {
							Log.e(TAG,
									"Volume Increment Exception:"
											+ e.getMessage());
						}
					}
				});
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		try {
			increment = Integer.parseInt(backend.getPrefs().getString(
					getResources().getString(R.string.pref_volumeincrement),
					"5"));
			...
			case R.id.control_menu_local:
				try {
					...
					i.setAction(MediaStore.INTENT_ACTION_MEDIA_SEARCH)
							...
							.putExtra(MediaStore.EXTRA_MEDIA_TITLE,
									status.getTrackName())
							.putExtra(MediaStore.EXTRA_MEDIA_FOCUS,
									MediaStore.Audio.Media.TITLE);
					startActivity(i);
				} catch (ActivityNotFoundException anfe) {
					Toast.makeText(this, "Cannot find an app to fulfil request",
